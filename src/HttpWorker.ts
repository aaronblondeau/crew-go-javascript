...
export default abstract class HttpWorker {
  ...
  async serve() {
    this.group?.server?.app.post('/execute/' + this.id, this.authMiddleware, async (req: express.Request, res: express.Response) => {
  ...
  async authMiddleware(req: express.Request, res: express.Response, next: express.NextFunction) {
    const requiredToken = process.env.CREW_WORKER_AUTH_TOKEN || ''
    if (requiredToken) {
